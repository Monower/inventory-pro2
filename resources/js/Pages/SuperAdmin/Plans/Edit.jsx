...
export default function Edit({ plan }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route("super-admin.plans.update", plan.id));
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Edit Plan - ${company_name}`} />
            <section className="space-y-6">
                <div className="flex items-center gap-4">
                    <BackButton url="super-admin.plans.index" />
                    <div>
                        <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100">Edit billing plan</h1>
                        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Adjust plan pricing, trial window, and availability.</p>
                    </div>
                </div>
